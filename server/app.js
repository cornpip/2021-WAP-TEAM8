...
db.query(
  `select id from insertproduct where user='test1'`,
  function (err, result) {
    //console.log(result);
  ...
  s.on("connection", (ws) => {
    ws.on("message", (message) => {
      ...
      if (parse.type == "leaveinfo") {
        db.query(
          `select inglist from user where id='${parse.userid}'`,
          function (err1, result1) {
            db.query(
              `select remainder from inguserlist where id=${parse.productid}`,
              function (err2, result2) {
                db.query(
                  `select nowuser from insertproduct where id = ${parse.productid}`,
                  function (err3, result3) {
                    let nowuser = result3[0].nowuser - 1;
                    if (nowuser >= 1) {
                      db.query(
                        `update insertproduct set nowuser=${nowuser} where id=${parse.productid}`
                      );
                    }
                  }
                );
                ...
                db.query(
                  `update user set inglist='${userarr}' where id='${parse.userid}'`
                );
                db.query(
                  `update inguserlist set remainder='${remainarr}' where id='${parse.productid}'`
                );
                db.query(sql2, [
                  parse.productid,
                  cport,
